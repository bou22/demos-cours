...
// #2 Le token CSRF est vérifié et validé. 

        //Pour retirer la protection csrf: commenter ce if et son else
        if (!empty($_POST['autorisation']) && formTokenValide(filter_input(INPUT_POST,'autorisation',FILTER_SANITIZE_SPECIAL_CHARS)))
        { 
            ...
            file_put_contents("./erreurs-access.log","[".$_SERVER['REMOTE_ADDR']."]  ".date("d F Y H:i:s")."  Token CSRF absent ou erroné. - REFERER: ".$_SERVER['HTTP_REFERER']." - ".$_SERVER['REQUEST_URI']." - "."\n",FILE_APPEND);
